...
fun myFunction(myValue: String, myValues: List<String>) {
    if (myValue owns 'c') { }
    if (myValue !in myValues) { }
    if (myValue > myValues.first()) { }
}
...
open class MyClass {
    ...
    protected open fun foo(): Unit? {
        val values = listOf(true, null, false);
        println(values);
        return null;
    }
...
data class MyDataClass(val num: Int) {
    public operator infix fun plus(to: Int) {
        return this + to;
    }
};